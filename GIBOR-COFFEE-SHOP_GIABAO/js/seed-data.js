...
(function () {
  ...
  function generateOrders(users) {
    ...
    for (let i = 1; i <= 70; i++) {
      ...
      // Trạng thái đơn hàng - đảm bảo đa dạng đủ các loại
      let status;
      if (i <= 15) status = "Hoàn tất";        // 15 đơn hoàn tất
      else if (i <= 25) status = "Đang xử lý"; // 10 đơn đang xử lý
      else if (i <= 35) status = "Đang giao";  // 10 đơn đang giao
      else if (i <= 50) status = "Đã ghi nhận"; // 15 đơn đã ghi nhận
      else if (i <= 60) status = "Đã hủy";     // 10 đơn đã hủy
      else status = "Chờ thanh toán";            // 10 đơn chờ thanh toán
  ...
  document.addEventListener("DOMContentLoaded", function () {
    try {
      ...
      // Chỉ tự động seed nếu chưa có user thường nào
      if (regularUsers.length === 0) {
        console.log("📋 Phát hiện chưa có dữ liệu mẫu. Tự động sinh...");
        seedAllData();
      } else {
        console.log(`📋 Đã có ${regularUsers.length} user. Bỏ qua auto-seed.`);
